...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='config/s3dis/s3dis_hyperedgeconv.yaml', help='config file')
    parser.add_argument('opts', help='see config/s3dis/s3dis_hyperedgeconv.yaml for all options', default=None, nargs=argparse.REMAINDER)
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if args.arch == 'hyperedgeconv':
        from model.hyperedgeconv.hyperedgeconv import hyperedgeconv as Model
    else:
    ...
    if args.distributed:
        ...
        model = torch.nn.parallel.DistributedDataParallel(
            model.cuda(),
            device_ids=[gpu],
            find_unused_parameters=True if "hyperedge" in args.arch else False
        )
